...
**TODO**: Organize the order of these module constructions.
...
**TODO**: Rename `covariant` to `FUNCTOR`.

`covariant` copies the sort and subsort structure without linking the original and new sort structures.
...
`covariant-data-up/down` link the original and new sort structures, with the new sorts as super/sub-sorts, respectively.

**TODO**: Rename to `FUNCTOR-ABOVE` and `FUNCTOR-BELOW`.
...
Here, `covariant-data-binary` uses `covariant-data-up` to build the desired sort structure, then the appropriate binary operators with unit are declared.

**TODO**: Rename to `BINARY-DATA`.
...
**TODO**: Move this towards the beginning of `MODULE-CONSTRUCTION` where they are first defined.
