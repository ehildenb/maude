...
private:
  ...
  {
    //
    //	We postpone dealing with returns generated by another return
    //	until we can check if they are the bottom of a Deterministic Reduction Path.
    //
    int ruleNr;
    int startTokenNr;
  };
  ...
  void buildExpansionTable(int i);
  void buildExpansion(int i, int prec);
  ...
  void buildDeterministicReductionPathTree(int endToken, int triggerRet, const Vector<DeferredReturn>& drp, int drpIndex);
