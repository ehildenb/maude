...
Parser::extractNextParse()
{
  ...
  p.nextReturnToCheck = r.nextReturn;
  //p.endToken = lastIndex;  // no change
  p.firstSon = NONE;
  p.nextSibling = NONE;
  extractFirstSubparse(0);
  return true;
	  ...
	  if (retPair.second)
	    {
	      ...
	      //
#ifdef PARSER_DEBUG
	      FOR_EACH_CONST(k, Vector<DeferredReturn>, drp)
...
Parser::extractNextSubparse(int nodeNr)
{
  ...
  for (; son != NONE; ++pos)
      ...
      if (extractNextSubparse(son))
	{
      ...
      int endTokenNr = parseTree[son].endToken;
      //
